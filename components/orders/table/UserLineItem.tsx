...
const UserLineItem = (props: IProp) => {
  ...
  return (
    <tr className="h-min text-[16px] text-[#000000] font-[400] leading-[22.4px] relative">
      ...
      <td className={`td2 text-[#35C6F4]`}>{props.row.customer_id}</td>
      <td className={`td3 text-[#35C6F4]`}>{props.row.email}</td>
      <td className={`td4`}>{props.row.phone}</td>
      <td className={`td5`}>{getDateInStringFormat(props.row.created_on)}</td>
      <td className={`td6 `} style={{}}>
        {props.row.age}
      </td>
      <td className={`td7 `}>{genderHanlder(props.row.gender)}</td>
      <td className={`td8 `}>
