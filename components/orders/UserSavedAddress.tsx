...
const UserSavedAddress = (props: { address: IAddressProps; register?: any; edit: (id: number) => void; update: () => Promise<IAddressProps[] | undefined> }) => {
    ...
    const deleteAddressHandler = async () => {
        ...
        if (user?.is_logged_in_users) {
            ...
            const result = await fetchJson(`/api/addresses?id=${props.address.id_addresses}`, {
                method: "DELETE",
            });
            // console.log(result)
    ...
    return (
        <div className="transition duration-300 flex items-start gap-x-[7px] border-[0.4px] border-[#BBC2CF] hover:bg-[#EDF5F9] rounded-[4px] p-[25px] h-full">
            <input
                type="radio"
                name="address"
                // defaultChecked={
            ...
            <div className="flex-1 flex flex-col justify-center h-full">
                <div className="flex items-start justify-between">
                    ...
                    {user?.default_address_users === props.address.id_addresses && (
                        <div className="bg-[#FF645A] rounded-[4px] text-[10px] text-[#FFFFFF] font-[500] leading-[15px] py-[5px] px-[10px]">{content[0]}</div>
                    )}
                </div>
                ...
                <div className="flex-type1 mt-[15px] gap-x-[10px]">
                    ...
                    <div className="text-[12px] text-[#2B2B2B] font-[500] leading-[17px]">{props.address.phone_addresses}</div>
                </div>

                <div className="text-[12px] text-[#3672DF] font-[500] leading-[17px] flex justify-end flex-1 grow">
                    <div className="gap-x-[20px] flex items-end">
