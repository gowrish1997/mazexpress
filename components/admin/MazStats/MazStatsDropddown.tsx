...
const MazStatsDropddown = (props: IProp) => {
   ...
   props.onChange?.(value)
    ...
    useEffect(() => {
        setCurrentValue(props.options[0].label as string);
    }, []);

    return (
        <div className="relative z-40">
            <div
                className={"border-[1px] border-[#BBC2CF] rounded-[4px] text-[12px] text-[#2B2B2B] font-[600] leading-[18px] box-border px-[10px] py-[5px]  hover:bg-[#BBC2CF] hover:text-[#FFFFFF] tracking-wider disabled:opacity-50 flex flex-row justify-between items-center space-x-[5px] relative cursor-pointer"+" "+props.className}
                style={showDropdown ? { backgroundColor: "#3672DF", color: "#FFFFFF" } : {}}
                onClick={toggleDropdownHandler}
            >
                <span className="capitalize">{`${props.type}:  ${currentValue}`}</span>
