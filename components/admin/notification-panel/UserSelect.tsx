  ...
  const updateSelectedUsers = (e: any, id: string) => {
    ...
    if (e.target.checked) {
      ...
      if (props.selectedUsers?.find((item) => item === id)) {
        ...
        if (props.selectedUsers?.length === (users.data as User[]).length - 1) {
          setMarkAll(true);
        }
      ...
      if (props.selectedUsers.find((item) => item === id)) {
        // delete
        props.setSelectedUsers((prev) => prev.filter((item) => item !== id));
      }
    }
  };
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {!showDropdown && props.selectedUsers.length > 0 && !markAll && (
          <div className="flex items-center w-full h-[45px] p-3">
              ...
              <Image
                src={
                  (users?.data as User[])?.find(
                    (el) => el.email === props.selectedUsers[0]
                  )?.avatar_url
                    ? `https://api.mazexpress.com.ly/user/` +
                      (users?.data as User[])
                        ?.find((el) => el.email === props.selectedUsers[0])
                        ?.avatar_url?.replace(/['"]+/g, "")
            ...
            <p className="text-[#2B2B2B] text-[14px] mx-2">
              {
                (users?.data as User[])?.find(
                  (item) => item.email === props.selectedUsers[0]
                )?.first_name
              }{" "}
              {
                (users?.data as User[])?.find(
        ...
        {!showDropdown && !markAll && props.selectedUsers.length === 0 && (
          <div className="flex items-center w-full h-[40px] p-2">
                  ...
                  <div className="flex items-center w-full" key={el.id}>
                    ...
                    <div className="">
                      <input
                        ...
                        className="w-3 h-3"
                        checked={
                          props.selectedUsers.find((seel) => seel === el.email)
                            ? true
                            : false
                        }
