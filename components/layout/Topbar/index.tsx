...
const Topbar = () => {
    ...
    const [unreadNotification, setUnreadNotification] = useState<INotification[]>();

    useEffect(() => {
      console.log('useEddefasd')
        // if (notifications && notifications?.length > 0) {
        //     const filteredNotification = notifications?.filter((data) => {
        //         return data.status_notifications == "unread";
        //     });
        //     console.log(filteredNotification);
        //     setUnreadNotification(filteredNotification);
        // }
    }, [notifications]);
    ...
    return (
        <>
            <div className="flex w-full min-h-[60px] py-5 items-center justify-between">
                ...
                <div className="flex min-h-[65px] items-center justify-end" onClick={togglingNotificationHandler}>
                    ...
                    {user && user.avatar_url_users ? (
                        <Image src={"/user-images/" + user?.avatar_url_users!} className="rounded-full" width={31} height={30} alt="profileImage" />
                    ) : (
            ...
            <NotificationView close={togglingNotificationHandler} show={showNotification} notificatoin={notifications!} />
        </>
    );
};
