...
const ProfilePicPop = (props: IProp) => {
    ...
    const profilePicPopContent: string[] = t("settingsPage.profileForm.ProfilePicPopContent", { returnObjects: true });
    ...
    const updateUserImage = (e: ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            const fileName = nanoid() + "." + String(e.target.files[0].name).split(".").pop();
            // dev
                ...
                <div className="box-border fixed top-0 left-0 w-[100vw] h-[100vh] bg-[rgba(0,0,0,0.4)] flex flex-row justify-center items-center z-50">
                    ...
                    <div className="flex flex-col bg-[#ffffff] rounded-[8px] py-[20px] px-[25px] w-[600px] gap-y-[15px]">
                        <div className="flex items-center justify-between text-[#525D72]">
                            <h1 className="font-[600] text-[16px]">{profilePicPopContent[0]}</h1>
                            <button className="w-7 h-7 flex items-center hover:bg-[#EDF5F9] p-2 rounded-full transition duration-300" onClick={props.close}>
